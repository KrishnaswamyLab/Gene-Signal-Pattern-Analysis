...
def train(args):
    ...
    print ('Load data...')
    trajectory_data = np.load('./data/splatter_simulated_data_3_branches.npz')
    ...
    if args.task == 'coexpression':
        ...
        signals = np.load('./data/localization_signals_3_branches.npz')['signals']
        signals = signals / np.linalg.norm(signals, axis=1).reshape(-1,1)
    ...
    np.savez_compressed(f'./results/{args.task}/{args.model}/{args.save_as}_results_3_branches.npz', signal_embedding=results['signal_embedding'], localization_score=results['localization_score'], config=vars(args))
